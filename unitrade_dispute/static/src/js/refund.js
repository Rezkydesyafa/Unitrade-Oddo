...
publicWidget.registry.UnitradeOrderRefundModal = publicWidget.Widget.extend({
  ...
  _onOpen(ev) {
    ...
    this._setValue(
      '[data-refund-ledger-id]',
      button.dataset.refundLedgerId || '',
    );
    this._setValue('[data-refund-line-id]', button.dataset.refundLineId || '');
    this._setText(
      '[data-refund-product-name]',
      button.dataset.refundProductName || 'Produk',
    );
    this._setText(
      '[data-refund-seller-name]',
      button.dataset.refundSellerName || 'Penjual UniTrade',
    );
  ...
  _syncUnboxingRequirement() {
    ...
    if (driveHint) {
      driveHint.textContent =
        'Wajib. Upload video unboxing ke Google Drive, lalu tempel link yang bisa diakses untuk proses review.';
    }
    if (videoHint) {
  ...
  _onDropzoneDrop(ev) {
    ...
    const root = this._root();
    const input =
      zone.dataset.refundDropzone === 'video'
        ? root.querySelector('[data-refund-unboxing]')
        : root.querySelector('[data-refund-photos]');
  ...
  _validateVideo(input) {
    ...
    files.forEach((file) => {
      ...
      if (file.size > VIDEO_MAX_SIZE) {
        errors.push(
          `${file.name} melebihi 10 MB. Upload ke Google Drive lalu isi link di bawah.`,
        );
      }
    });
    this._setUploadError(input.closest('[data-refund-dropzone]'), errors);
  ...
  _clearUploadState() {
    ...
    this.photoFiles = [];
    root
      .querySelectorAll('[data-refund-photos], [data-refund-unboxing]')
      .forEach((input) => {
        input.value = '';
      });
  ...
  _revokePhotoPreviews() {
    this.photoPreviewUrls.forEach((previewUrl) =>
